  ...
  return (
    ...
    >
      ...
      >
        {items.map((item) => (
          <li
            className="relative w-[350px] max-w-full shrink-0 rounded-2xl border border-b-0 bg-white px-8  py-6 text-black md:w-[450px]"
            key={item.name}
          >
            <blockquote>
              <div
              ...
              ></div>
              <span className=" s relative z-20 text-base font-normal leading-[1.6]">{item.quote}</span>
              <div className="relative z-20 mt-6 flex flex-row items-center">
                <span className="flex flex-col gap-1">
                  <span className=" text-sm font-normal leading-[1.6]">{item.title}</span>
                  <span className=" text-sm font-bold  leading-[1.6]">{item.name}</span>
                </span>
              </div>
            </blockquote>
          </li>
